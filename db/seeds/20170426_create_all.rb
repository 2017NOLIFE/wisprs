...
Sequel.seed(:development) do
  def run
    puts 'Seeding accounts, public keys, messages'
    create_accounts
    create_public_key
    create_messages
  end
...
def create_public_key
  public_key = ALL_PUBLIC_KEY_INFO.each
    loop do
      ...
      CreatePublicKeyForAccount.call(owner_id: account.id,
                                     key: public_key_item[:key],
...
def create_messages
  messages = ALL_MESSAGES_INFO.each
    loop do
      ...
      SendMessage.call(from_id: sender.id,
                       to_id: receiver.id,
                       title: messages_item[:title],
                       about: messages_item[:about],
                       expire_date: messages_item[:expire_date],
                       status: messages_item[:status],
                       body: messages_item[:body])
    end
end
